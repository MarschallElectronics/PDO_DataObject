	...
	*/
	...
	class PDO_DataObject_Generator extends PDO_DataObject
	{
		...
		private static $config = array(
			...
			'var_keyword'           => 'public',
			// var|public  - (or private iadd_database_nicknamef you want to break things)
		...
		function __construct($cfg = false)
		{
			...
			{
				class_exists($hook) ? '' : require_once 'PDO/DataObject/Generator/Hooks.php';
			}
		 ...
		 * @param bool $table
		 * @return array|false (varies) - depends if you are setting or getting...
		 * @throws PDO_DataObject_Exception
		 */
		...
		function readTableList()
		{
			...
			{
				$views = $io->getListOf(is_string($options['build_views']) ? $options['build_views'] : 'views');
		...
		function newTable($name)
		{
			...
			{
				class_exists($tcls) ? '' : require_once 'PDO/DataObject/Generator/Table.php';
			}
		...
		function generateIni()
		{
			...
			if (is_array($base))
			{
				...
				{
					$this->raise("Could not find schema location from config[schema_location] - array but no matching database", PDO_DataObject::ERROR_INVALIDCONFIG);
				}
			...
			if (!$fh)
			{
				return $this->raise("Failed to create temporary file: $tmpname\n" . "make sure session.save_path is set and is writable\n", null);
			}
			fwrite($fh, $out);
			fclose($fh);
