<?php
	 ...
	 *
	 * @author Benedict Reuthlinger : diverse Stellen umprogrammiert
	 */
	class PDO_DataObject
	{
		...
		const STR = 2;  // does not require ''
		
		const DATE = 4;  // requires ''
		
		const TIME = 8;  // is date
		
		const BOOL = 16;  // is time
		
		const TXT = 32; // is boolean
		
		const BLOB = 64; // is long text
		
		const NOTNULL = 128; // is blob type
		
		const MYSQLTIMESTAMP = 256;           // not null col.
		 ...
		 */
		const WHEREADD_ONLY = true;           // mysql timestamps (ignored by update/insert)
		...
		const ERROR_INVALIDARGS = 'InvalidArgs';  // from Pear DB compatibility options..
		
		const ERROR_NODATA = 'NoData';  // wrong args to function
		
		const ERROR_INVALIDCONFIG = 'InvalidConfig';  // no data available
		
		const ERROR_NOCLASS = 'NoClass';  // something wrong with the config
		
		const ERROR_SET = 'Set';  // no class exists
		
		const ERROR_QUERY = 'Query';  // set() caused errors when calling set*** methods.
		
		const ERROR_CONNECT = 'Connect';  // if query throws an error....
		 ...
		 *
		 */
		public static $set_errors = false;  // if connect throws an error....
		...
		private static $config = array(
			...
			// nickname überschreiben // by bene
			'nickname' => false,
		 ...
		 */
		private static $connections = array(); // set by config() and debugLevel()
		
		/**
		 * @var array
		 */
		private static $ini = array(); // md5 map of connections to DSN
		 ...
		 * @var  string
		 */
		public $_database_nickname = false;  // database table
		 ...
		 * @var     string
		 */
		private $_database_dsn_md5 = '';  // Number of rows returned from a query
		...
		private $_query = array(
			...
			'useindex'      => '',
			// the USE INDEX condition
			'limit_start'   => '',
			// the LIMIT condition
			'limit_count'   => '',
			// the LIMIT condition
			'data_select'   => false,
			// the columns to be SELECTed (false == '*') the default.
			'derive_table'  => '',
		...
		static function config($cfg_in = array(), $value = false)
		{
			...
			if (!is_array($cfg_in) && func_num_args() < 2)
			{
				...
				(new PDO_DataObject())->raise("Invalid Call to config should be string+anther value or array", self::ERROR_INVALIDARGS);
			}
			...
			foreach ($cfg as $k => $v)
			{
				...
				{
					(new PDO_DataObject())->raise("Invalid Configuration setting : $k", self::ERROR_INVALIDCONFIG);
				}
				self::$config[ $k ] = $v;
			}
			if (isset($cfg['debug']))
		...
		static function factory($in_table = '')
		{
			...
			if (!$rclass && self::$config['proxy'])
			{
				...
				$proxyMethod = (is_string(self::$config['proxy']) && strpos(self::$config['proxy'], '::') !== false) ? explode('::', self::$config['proxy'])[1] : ('getProxy' . self::$config['proxy']);
				...
				{
					$do->raise("Generator does not have method $proxyMethod, \n" . "  usually proxy should be set to 'Full', you set it to " . var_export(self::$config['proxy'], true), self::ERROR_INVALIDCONFIG);
				}
			...
			if (!$rclass || !class_exists($rclass))
			{
				...
				return $dor->raise("factory could not find class " . (is_array($rclass) ? implode(PATH_SEPARATOR, $rclass) : $rclass) . "from
                $table", self::ERROR_NOCLASS);
			}
		...
		static function debug($message, $logtype = 0, $level = 1)
		{
			...
			if (!self::$debug || (is_numeric(self::$debug) && self::$debug < $level))
			{
				return;
			}
		...
		static public function _is_null_member($obj_or_ar, $prop)
		{
			...
			switch (true)
			{
				...
				default:
					self::raise("argument passed to is_null_member was not an object or array", self::ERROR_INVALIDARGS);
			}
		...
		private static function tableToClass($table)
		{
			...
			if (!empty($database) && empty(self::$config['databases'][ $database ]))
			{
				...
				$do->raise("unable to find databases[{$database}] in Configuration, It is required for factory with database", self::ERROR_INVALIDARGS);
			}
		...
		private static function loadClass($class, $table = false)
		{
			...
			{
				$dor = new PDO_DataObject();
				return $dor->raise("autoload: Included $file \n   however could not find the class :" . implode(',', $cls), self::ERROR_NOCLASS);
		 ...
		 * @param $arguments
		 * @return mixed
		 * @author Bene Reuthlinger
		 */
		public function __call($name, $arguments)
		{
			$fdrei = strtolower(substr($name, 0, 3));
			$last  = substr($name, 3);
			
			if (strlen($last) <= 0)
				return null;
			
			// Klassen-Vars prüfen ob vorhanden (Groß-/Kleinschreibung ignorieren)
			$thisarr = (array)$this;
			foreach ($thisarr as $key => $val)
			{
				if (strtolower($key) == strtolower($last))
				{
					$last = $key;
					break;
				}
		...
		final function PDO()
		{
			...
			// nickname überschreiben -> als config übergeben, damit {structure}.ini immer gleich heißt // by bene
			if (empty($this->_database_nickname) && !empty(self::$config['nickname']))
			{
				$this->_database_nickname = self::$config['nickname'];
			}
			
			// nickname by tableName ??
			$tn = $this->tableName();
			...
			if (!$dsn)
			{
				return $this->raise("No database name / dsn found anywhere", self::ERROR_INVALIDCONFIG);
			...
			switch ($dsn_ar['scheme'])
			{
				...
				case 'sqlsrv':
					$pdo_dsn                 = $dsn_ar['scheme'] . ':' . 'Database=' . substr($dsn_ar['path'], 1) . (empty($dsn_ar['host']) ? '' : ';Server=' . $dsn_ar['host']) . (empty($dsn_ar['port']) ? '' : ',' . $dsn_ar['port']);
				...
				default:
					...
					$pdo_dsn                 = $dsn_ar['scheme'] . ':' . 'dbname=' . substr($dsn_ar['path'], 1) . (empty($dsn_ar['host']) ? '' : ';host=' . $dsn_ar['host']) . (empty($dsn_ar['port']) ? '' : ';port=' . $dsn_ar['port']);
					$dsn_ar['database_name'] = substr($dsn_ar['path'], 1);
					break;
			...
			$dsn_ar['nickname'] = empty($this->_database_nickname) ? $dsn_ar['database_name'] : $this->_database_nickname;
			...
			switch ($drv)
			{
				...
				case 'sqlsrv':
					...
					if (!is_numeric($start))
						$start = 0;
					if (!is_numeric($count))
						$this->raise("Limit-Query: Mssql: \$count has NO numeric Value!", self::ERROR_INVALIDARGS); // from PEAR DB?
				...
				case 'oci':
					if ($manip)
					{
						$this->raise("Oracle may not support offset in modification queries", self::ERROR_INVALIDARGS); // from PEAR DB?
					}
				...
				default:
					$this->raise("The Database $drv, does not support limit queries - if you know how this can be added, please send a patch.", self::ERROR_INVALIDARGS); // from PEAR DB?
			}
		}
		...
		{
			return $autojoin ? $this->factorySelf()->joinAll()->load($this->pid()) : $this->factorySelf()->load($this->pid());
		}
		...
		final function pid()
		{
			...
			if (!$keys)
			{
				...
				return $this->raise("No Primary Key available for table '{$this->tableName()}'", self::ERROR_INVALIDCONFIG);
				
			}
			$k = $keys[0];
			if (empty($this->$k))
			{ // we do not
				return $this->raise("pid() called on Object where primary key value not available", self::ERROR_NODATA);
			}
			return $this->$k;
		}
		...
		final function toSelectSQL($unions = true)
		{
			...
			if (!empty($this->_query['derive_table']) && !empty($this->_query['derive_select']))
			{
				...
				$sql = 'SELECT ' . $this->_query['derive_select'] . " \n" . "FROM ( $sql ) " . $this->_query['derive_table'];
		...
		final function find($n = false)
		{
			...
			{
				return $this->raise("You cannot do two queries on the same object (copy it before finding)", self::ERROR_INVALIDARGS);
				return false;
			}
			...
			{
				return $this->raise("NO \$__table SPECIFIED in class definition", self::ERROR_INVALIDARGS);
			...
			// find(true)
			if ($n && ($this->N > 0 || $this->N === true))
			{ // technically in sqlite N=true therefore N>0..
				if (self::$debug)
				{
        ...
        */
			...
			if (!$array)
			{
				...
				if (self::$debug)
				{
					...
					$this->debug("Last Data Fetch'ed after " . number_format($t[0] + $t[1] - $this->_result->time_query_end, 3) . " seconds", __FUNCTION__, 2);
				}
			...
			if (!isset($this->_result->fields))
			{
				...
				for ($i = 0; $i < $this->_result->columnCount(); $i++)
				{
					...
					if (!$meta)
					{ // not sure what is actually returned if it's not supported by the driver..
						break;
					}
					$k  = $meta['name'];
					$kk = (strpos($k, '.') === false && strpos($k, ' ') === false) ? $k : str_replace($replace, '_', $k);
					...
					{ // tried using 'native_type' - just returns junk!
		...
		final function whereAdd($cond = false, $logic = 'AND')
		{
			...
			{
				return $this->raise("You cannot do two queries on the same object (clone it before finding)", self::ERROR_INVALIDARGS);
			}
		...
		final function having($having = false, $logic = 'AND')
		{
			...
			if (!in_array($logic, array(
				'AND',
				'OR'
			)))
			{
				return $this->raise("Having logic should be AND or OR", self::ERROR_INVALIDARGS);
			}
		...
		final function selectAs($from = null, $format = '%s', $tableName = false)
		{
			...
			if ($this->_query === false)
			{
				$this->raise("You cannot do two queries on the same object (copy it before finding)", self::ERROR_INVALIDARGS);
				return false;
			}
		...
		final function insert()
		{
			...
			if (!$items)
			{
				return $this->raise("insert:No table definition for {$this->tableName()}", self::ERROR_INVALIDCONFIG);
				return false;
			}
			...
			foreach ($items as $k => $v)
			{
				...
				{
					$this->raise("Trying to set {$k} to null however it's set as NOT NULL", self::ERROR_INVALIDARGS);
				}
				...
				if (!$this->$k && (($v & self::DATE) || ($v & self::TIME)) && !($v & self::NOTNULL))
				{
				...
				if ($v & self::STR)
				{
					$rightq .= $PDO->quote((string)(($v & self::BOOL) ? // this is thanks to the braindead idea of postgres to
							// use t/f for boolean.
				...
				{
					$this->raise("Trying to insert a row - found object used as a column variable " . print_r(array(
							$k,
							$this->$k
						), true), self::ERROR_INVALIDARGS);
					continue;
				}
			...
			if (empty($leftq) && in_array($dbtype, array(
					'pgsql',
					'sqlite'
				)))
			{
			...
			if ($key && $useNative)
			{
				switch ($dbtype)
				{
					...
					case 'pgsql':
						if (!$seq)
						{
							$this->raise("Could not determine Sequence name for table: " . $this->tableName(), self::ERROR_INVALIDCONFIG);
						}
		...
		final function update($dataObject = false)
		{
			...
			if (!strlen($where))
			{
				return $this->raise("updating all records in a table is not enabled by default, use \$do->whereAdd('1=1'); if you really want to do that.", self::ERROR_INVALIDARGS);
			}
			...
			if (!$settings)
			{
				$this->raise("update: No Data specifed for query $settings , {$this->_query['condition']}", self::ERROR_NODATA);
			}
		...
		public function delete($useWhere = false)
		{
			...
			$where = !empty($this->_query) && !empty($this->_query['condition']) ? $this->_query['condition'] : '';
			...
			if (!strlen($where))
			{
				return $this->raise("deleting all data from database is disabled by default, use where('1=1') if you really want to do that.", self::ERROR_INVALIDARGS);
			}
			
			
			$table = ($quoteIdentifiers ? $this->quoteIdentifier($this->tableName()) : $this->tableName());
			$sql   = "DELETE " . ((!empty($this->_join) && $useWhere) ?  // using a joined delete. - with useWhere..
					"{$table} FROM {$table} {$this->_join} " : "FROM {$table}") . " WHERE " . $where . $extra_cond;
		...
		final function count($countWhat = false, $whereAddOnly = false)
		{
			...
			if ($t->_query === false)
			{
				return $this->raise("You cannot do run count after you have run fetch()", self::ERROR_INVALIDARGS);
			}
			...
			{
				return $this->raise("You cannot do run count without keys - use \$do->count('id'), or use \$do->count('distinct id')';", self::ERROR_INVALIDARGS);
			}
			$table   = ($quoteIdentifiers ? $this->quoteIdentifier($this->tableName()) : $this->tableName());
			...
			$r = $t->query("SELECT count({$countWhat}) as $as
		...
		final function query($string)
		{
			...
			switch (true)
			{
				...
				default:
					...
					// Wporkaround für scheiss rowCount-Kacke -> https://secure.php.net/manual/de/pdostatement.rowcount.php#113608
					if (strtolower(substr(trim($string), 0, 6)) == 'select')
					{
						$result__ = $pdo->prepare($string, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
						$result__->execute();
						$no_results = $result__->rowCount();
					}
					else
					{
		...
		final function databaseStructure($database_nickname = false, $inidata = false, $linksdata = false, $overwrite = false)
		{
			...
			if (count($args) > 1)
			{
				...
				if ($inidata !== false)
				{
					...
					self::$ini[ $database_nickname ] = isset(self::$ini[ $database_nickname ]) && !$overwrite ? self::$ini[ $database_nickname ] + $inidata : $inidata;
				}
				if ($linksdata !== false)
				{
					if (self::$debug)
					...
					self::$links[ $database_nickname ] = isset(self::$links[ $database_nickname ]) && !$overwrite ? self::$links[ $database_nickname ] + $linksdata : $linksdata;
				}
			...
			if (is_array(self::$config['schema_location']))
			{
				...
				{
					$this->raise("Could not find configuration for database $database_nickname in schema_location", self::ERROR_INVALIDCONFIG);
				}
				
				$schemas = is_array(self::$config['schema_location'][ $database_nickname ]) ? self::$config['schema_location'][ $database_nickname ] : explode(PATH_SEPARATOR, self::$config['schema_location'][ $database_nickname ]);
			}
			...
			{
				$this->raise("Invalid format or empty value for config[schema_location]", self::ERROR_INVALIDCONFIG);
			}
			...
			foreach ($schemas as $ini)
			{
				...
				$ini_out = array_merge($ini_out, parse_ini_file($fn, true));
			...
			$this->raise("Unable to load schema for database and table (turn debugging up to 5 for full error message)", self::ERROR_INVALIDARGS);
			return false;
		...
		function generator()
		{
			...
			$cls  = (is_string(self::$config['proxy']) && strpos(self::$config['proxy'], '::') !== false) ? explode('::', self::$config['proxy'])[0] : $dcls;
			
			if ($dcls == $cls)
			{
				class_exists('PDO_DataObject_Generator') ? '' : require_once 'PDO/DataObject/Generator.php';
			}
		...
		function tableName()
		{
			...
			$tbl = isset(self::$config['table_alias'][ $this->__table ]) ? self::$config['table_alias'][ $this->__table ] : $this->__table;
		...
		function keys()
		{
			...
			foreach ($keys as $k)
			{
				if (!isset($cols[ $k ]))
				{
					$this->raise("Schema for {$this->tableName()} is not valid, key '{$k}' does not exist in the table structure information", self::ERROR_INVALIDCONFIG);
				}
			}
			return $keys;
		}
		...
		function sequenceKey()
		{
			...
			if (!$keys)
			{
				return self::$sequence[ $dn ][ $tn ] = array(
					false,
					false,
					false
				);
			...
			if (in_array($dbtype, array('pgsql')) && ($table[ $usekey ] & self::INT) && isset($realkeys[ $usekey ]) && strlen($realkeys[ $usekey ]) > 1)
			{
				return self::$sequence[ $dn ][ $tn ] = array(
					$usekey,
					true,
			...
			if (in_array($dbtype, array(
					...
					'ifx',
					'sqlite'
				)) && ($table[ $usekey ] & self::INT) && isset($realkeys[ $usekey ]) && ($realkeys[ $usekey ] == 'N'))
			{
				return self::$sequence[ $dn ][ $tn ] = array(
					$usekey,
					true,
		...
		final function whereToString($keys = false, $filter = array(), $negative_filter = array(), $tableName = false)
		{
			...
			$ret = empty($this->_query) || empty($this->_query['condition']) ? '' : trim($this->_query['condition']);
			...
			foreach ($keys as $k => $v)
			{
				...
				$kSql = $quoteIdentifiers ? ($this->quoteIdentifier($tableName) . '.' . $this->quoteIdentifier($k)) : "{$tableName}.{$k}";
		...
		function links()
		{
			...
			foreach ($schemas as $ini)
			{
				...
				self::$links[ $dn ] = array_merge(self::$links[ $dn ], parse_ini_file($fn, true));
			...
			if (false === self::$links[ $dn ])
			{
				...
				return $this->raise("Failed to load any links schema for database={$this->_database_nickname} from these files/locations" . json_encode($tried), self::ERROR_INVALIDCONFIG);
			}
			...
			if (self::$config['portability'] & self::PORTABILITY_LOWERCASE)
			{
				foreach (self::$links[ $dn ] as $k => $v)
				{
					...
					$nk = strtolower($k);
					// results in duplicate cols.. but not a big issue..
					self::$links[ $dn ][ $nk ] = isset(self::$links[ $dn ][ $nk ]) ? self::$links[ $dn ][ $nk ] : array();
		...
		function joinAdd($obj = false, $joinType = 'INNER', $joinAs = false, $joinCol = false)
		{
			...
			if (!$ofield && ($olinks = $obj->links()))
			{
				...
				foreach ($olinks as $k => $linkVar)
				{
					...
					foreach ($linkVar as $v)
					{
						...
						if ($joinCol !== false)
						{
							$this->raise("joinAdd: You cannot target a join column in the " . "'link from' table ({$obj->tableName()}). " . "Either remove the fourth argument to joinAdd() " . "({$joinCol}), or alter your links.ini file. ", self::ERROR_INVALIDARGS);
							return false;
						}
			...
			if ($ofield === false)
			{
				return $this->raise("joinAdd: {$obj->tableName()} has no link with {$this->tableName()}", self::ERROR_INVALIDARGS);
			}
			...
			$objTable = $quoteIdentifiers ? $this->quoteIdentifier($obj->tableName()) : $obj->tableName();
			...
			{
				$dbPrefix = ($quoteIdentifiers ? $this->quoteIdentifier($obj_database) : $obj_database) . '.';
			}
			...
			if (get_class($obj) != 'PDO_DataObject')
			{
				...
				if (!$items)
				{
					return $this->raise("joinAdd: No table definition for {$obj->tableName()}", self::ERROR_INVALIDCONFIG);
				}
				...
				foreach ($items as $k => $v)
				{
					...
					if ($v & self::STR)
					{
						$obj->whereAdd("{$joinAs}.{$kSql} = " . $PDO->quote((string)(($v & self::BOOL) ? // this is thanks to the braindead idea of postgres to
								// use t/f for boolean.
				...
				if ($this->_query === false)
				{
					return $this->raise("joinAdd can not be run from a object that has had a query run on it,
                    clone the object or create a new one and use setFrom()", self::ERROR_INVALIDARGS);
				}
			}
			...
			if ($quoteIdentifiers)
			{
				...
				$ofield = (is_array($ofield)) ? array_map(array(
					$this,
					'quoteIdentifier'
				), $ofield) : $this->quoteIdentifier($ofield);
				$tfield = (is_array($tfield)) ? array_map(array(
					$this,
					'quoteIdentifier'
				), $tfield) : $this->quoteIdentifier($tfield);
			}
			...
			if ($addJoinAs)
			{
				...
				// for those databases.
				$fullJoinAs = in_array($PDO->getAttribute(PDO::ATTR_DRIVER_NAME), array(
					'mysql',
					'pgsql'
				)) ? "AS {$joinAs}" : $joinAs;
			}
		...
		function autoJoin($cfg = array())
		{
			...
			foreach ($map as $ocl => $info)
			{
				...
				{
					$this->raise("format of links.ini is not correct for table {$this->tableName()} - missing 'colon:' in value - " . print_R($map, true), self::ERROR_INVALIDCONFIG);
					continue;
				}
				...
				if (!empty($cfg['exclude']) && (in_array($tab . '.*', $cfg['exclude']) || in_array($ocl . '.*', $cfg['exclude']) || in_array('join_' . $ocl . '_' . $col, $cfg['exclude'])))
				{
					continue;
				}
		...
		final function setFrom($from, $format = '%s', $skipEmpty = false)
		{
			...
			if (!$items)
			{
				$this->raise("setFrom:Could not find table definition for {$this->tableName()}", self::ERROR_INVALIDCONFIG);
				return;
			}
		...
		function toArray($format = '%s', $hideEmpty = false)
		{
			...
			$ar = ($rf !== false) ? (($hideEmpty === 0) ? $rf : array_merge($rf, $this->tableColumns())) : $this->tableColumns();
		...
		function validate()
		{
			class_exists('PDO_DataObject_Validate') ? '' : require_once 'PDO/DataObject/Validate.php';
		...
		{
			return !$this->_result ? false : (is_a($this->_result, 'StdClass') ? false : $this->_result);
